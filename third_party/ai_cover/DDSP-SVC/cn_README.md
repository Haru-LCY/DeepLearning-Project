...
已知一个可运行组合是：python 3.8 (windows) + cuda 11.8 + torch 2.4.1 + torchaudio 2.4.1。

如果你是在 Linux 服务器或集群环境下做离线推理 / 训练，并且不需要图形界面，可以只使用 `preprocess.py`、`main_reflow.py`、`train_reflow.py` 这些 CLI 入口。
...
如果你需要图形界面，请自行查阅上游项目的其他发行版本或前端实现；当前仓库文档不再把 GUI 作为推荐入口。
