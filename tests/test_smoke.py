...
def test_expected_paths_exist() -> None:
    ...
    assert (repo_root / "scripts" / "run_expressive_render.py").exists()
    assert (repo_root / "scripts" / "run_pianist_transformer_stage.py").exists()
    ...
    assert (repo_root / "src" / "expressive_render.py").exists()
